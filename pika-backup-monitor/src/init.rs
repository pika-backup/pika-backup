...
fn on_startup(_app: &gio::Application) {
    ...
    crate::connect::init::init();
    crate::schedule::init::init();
    ...
    glib::MainContext::default().spawn(async {
        match ashpd::desktop::background::BackgroundProxy::new().await {
            ...
            Err(err) => tracing::error!("Error acquiring background proxy: {err:?}"),
        }

        common::utils::listen_remote_app_running(
            common::APP_ID,
            &crate::dbus::session_connection()
                .await
        ...
        if let Ok(monitor) = file.monitor_file(gio::FileMonitorFlags::NONE, gio::Cancellable::NONE)
        {
            monitor.connect_changed(|_monitor, _file, _other, event| {
                if event == gio::FileMonitorEvent::Created {
                    if APP_RUNNING.get() {
                        tracing::info!("Detected flatpak update. Not restarting monitor process, main app is still running.");
                    } else {
                        tracing::info!("Detected flatpak update. Restarting monitor process.");
