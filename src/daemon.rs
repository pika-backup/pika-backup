...
fn forward_action(action: &gio::SimpleAction, target_value: Option<&glib::Variant>) {
    debug!(
        "Forwarding action: {:?}",
        gio::Action::print_detailed_name(&action.get_name(), target_value)
    );
    ...
    group.activate_action(&action.get_name(), target_value);
}
            ...
            if let Some(uuid) = uuid {
                ...
                }) = backup
                {
                    ...
                    notification.add_button_with_target_value(
                        "Run Backup",
                        &format!("app.{}", crate::action::backup_start().get_name()),
