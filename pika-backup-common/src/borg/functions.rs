...
impl CommandRun<task::Mount> for Command<task::Mount> {
    async fn run(self) -> Result<()> {
        ...
        tracing::debug!("Ensuring mount directory exists: {dir:?}");
...
impl CommandRun<task::KeyChangePassphrase> for Command<task::KeyChangePassphrase> {
    async fn run(self) -> Result<()> {
        ...
        // that function.
        tracing::info!("Running borg: {:#?}", borg_call);
...
pub async fn umount(repo_id: &RepoId) -> Result<()> {
    ...
    if let Err(err) = smol::fs::remove_dir(mount_point).await {
        match err.kind() {
            std::io::ErrorKind::NotFound => {
                ...
                tracing::warn!("Error when removing mount dir: {:?}", err);
            }
    ...
    if let Err(err) = smol::fs::remove_dir(mount_base_dir()).await {
        tracing::debug!("Error when removing mount base dir: {:?}", err);
    }
