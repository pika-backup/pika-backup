...
impl Global {
    ...
    pub async fn check(
        ...
        let histories_with_repo_id = histories
            .iter()
            .filter(|(config_id, _)| {
                configs.try_get(config_id).map(|x| &x.repo_id) == Ok(&config.repo_id)
            })
...
mod test {
    ...
    fn test_check_real_hourly() {
        ...
        for activity in [activity_none, activity_enough] {
            for completed in [None, Some(yesterday), Some(now), Some(tomorrow)] {
                tracing::debug!("activity: {:?}, completed: {:?}", activity, completed);
    ...
    fn test_check_daily() {
        ...
        let due = Due::check(&config, Some(&history), None);
        assert!(match due {
            Err(Due::NotDue { next }) => {
        ...
        let due = Due::check(&config_close, Some(&history_close), Some(&activity));
        assert!(match due {
            Err(Due::NotDue { next }) => {
                assert_eq!(
                    next,
        ...
        let due = Due::check(&config_close, Some(&history), Some(&activity));
        assert!(match due {
            Err(Due::NotDue { next }) => {
                assert_eq!(
                    next,
