...
use super::imp;
use crate::prelude::*;
use crate::utils::notification::BackupNote;
...
impl imp::BackupPage {
    pub(super) async fn backup(
        ...
        let result = self.run_backup(config, from_schedule, guard).await;
        self.refresh_status();
    ...
    async fn run_prune(
        ...
        let prune_result = crate::utils::borg::exec(prune_command, guard)
            .await
            .into_borg_error()?;
        ...
        let compact_result = crate::utils::borg::exec(compact_command, guard)
            .await
            .into_borg_error()?;
    ...
    async fn run_backup(
        ...
        if main_ui().is_mapped() {
            ...
            glib::MainContext::default().spawn_local(async move {
                crate::toast_size_estimate::check(&config, communication).await
            });
        }
        ...
        // execute backup
        let result = crate::utils::borg::exec(command, guard).await;
        ...
        match result {
            ...
            Ok(_) => {
                ...
                let _ignore = main_ui()
                    ...
                    .await;
                let _ignore = crate::utils::df::lookup_and_cache(&config).await;
    ...
    async fn run_script(
        ...
        let result = crate::utils::borg::exec(command, guard).await;
        let outcome = match &result {
            Err(crate::error::Combined::Borg(borg::Error::Aborted(err))) => {
                Some(borg::Outcome::Aborted(err.clone()))
            }
            Err(crate::error::Combined::Borg(borg_err)) => Some(borg::Outcome::Aborted(
