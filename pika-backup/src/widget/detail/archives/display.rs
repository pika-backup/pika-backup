...
use crate::{App, backup_status};

impl imp::ArchivesPage {
    pub async fn show(&self) -> Result<()> {
        crate::utils::clear(&self.list);
    ...
    pub fn ui_display_archives(&self, repo_id: &borg::RepoId) {
        ...
        for (archive_name, archive) in repo_cache.archives_sorted_by_date() {
            ...
            row.add_row(&info(
                gettext("Duration"),
                &crate::utils::duration::plain(&(archive.end - archive.start)),
            ));
    ...
    pub async fn update_df(&self) -> Result<()> {
        ...
        if let Some(df) = crate::utils::df::cached_or_lookup(config).await {
