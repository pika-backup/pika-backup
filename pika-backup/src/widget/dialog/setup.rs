...
mod imp {
    ...
    impl SetupDialog {
        ...
        async fn show_add_existing_file_chooser(&self) -> Result<Option<gio::File>> {
            if let Some(path) =
                crate::utils::folder_chooser_dialog(&gettext("Setup Existing Repository"), None)
                    .await
                    .ok()
                    .and_then(|x| x.path())
            {
                self.obj().set_visible(true);
                if crate::utils::is_backup_repo(&path).await {
        ...
        async fn add_existing_repo(
            ...
            match result {
                ...
                Err(actions::ConnectRepoError::Error(crate::error::Combined::Ui(
                    crate::error::Error::UserCanceled,
                ))) => {
        ...
        async fn on_add_existing_page_error(&self, error: Option<&str>) {
            ...
            if let Some(error) = error {
                let error =
                    crate::error::Message::new(gettext("Failed to Configure Repository"), error);
        ...
        async fn save_password(
            ...
            if let Err(err) = crate::utils::password_storage::store_password(config, password).await
            {
