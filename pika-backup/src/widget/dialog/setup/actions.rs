    ...
    let mut repo = match location {
        ...
        SetupRepoLocation::Local(file) => {
            ...
            let path = if let Some(path) = file.path() {
                ...
                crate::repo::mount_enclosing(&mount_file).await?;
...
pub async fn try_peek(
    ...
    let result =
        crate::utils::borg::exec_repo_only(&gettext("Loading Backup Repository"), borg, |borg| {
            borg.peek()
        })
        .await;
...
pub async fn fetch_archive_list(config: &config::Backup) -> CombinedResult<Vec<borg::ListArchive>> {
    ...
    crate::utils::borg::exec(list_command, &guard).await
}
...
pub async fn init_new_backup_repo(
    ...
    crate::utils::borg::exec_repo_only(
    ...
    // Get repo id
    let info = crate::utils::borg::exec_repo_only(
