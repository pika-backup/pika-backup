...
mod imp {
    ...
    use super::*;
    use crate::widget::EncryptionSettings;
    ...
    pub struct PreferencesDialog {
        ...
        command_line_args_error: RefCell<Option<crate::error::Error>>,
        pre_backup_command_error: RefCell<Option<crate::error::Error>>,
        post_backup_command_error: RefCell<Option<crate::error::Error>>,
    ...
    impl PreferencesDialog {
        ...
        fn set_command_line_args(&self, args: String) {
            match crate::utils::borg::parse_borg_command_line_args(&args) {
                Ok(args) => {
        ...
        async fn test_run_script(
            ...
            if let Err(err) = crate::utils::borg::exec(command, &QuitGuard::default())
                .await
        ...
        async fn do_change_password_confirm(&self) -> Result<()> {
            ...
            self.change_password_communication
                .replace(Some(command.communication.clone()));
            crate::utils::borg::exec(command, &QuitGuard::default())
                .await
            ...
            if config.encrypted != encrypted {
                ...
                if !encrypted {
                    crate::utils::password_storage::remove_password(&config, true).await?;
                }
            }

            // Save to keyring
            if let Some(password) = password {
                crate::utils::password_storage::store_password(&config, &password).await?;
            }
